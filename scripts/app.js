...
// Build a rich lexicon context object for AI prompts (both prospect & closer modes)
function buildLexiconContext(lexicon, excuseLabel, prospectType) {
  const ctx = {
    excuseLabel: excuseLabel,
    layer1Past: lexicon.layer1 ? lexicon.layer1.past : [],
    layer1Present: lexicon.layer1 ? lexicon.layer1.present : [],
    layer1Variants: lexicon.layer1 ? lexicon.layer1.variants : [],
    layer2Patterns: lexicon.layer1 ? lexicon.layer1.layer2Patterns : [],
    typeSample: prospectType === 'A'
      ? (lexicon.simBehavior && lexicon.simBehavior.typeA)
      : (lexicon.simBehavior && lexicon.simBehavior.typeB),
    // Closer-mode-specific data
    scriptQuestions: [],
    paths: [],
    reframes: [],
    analogies: []
  ...
  // Pull SCRIPT questions
  if (lexicon.script && lexicon.script.letters) {
    Object.entries(lexicon.script.letters).forEach(([letter, data]) => {
      ctx.scriptQuestions.push({
        letter: letter,
        text: data.past || data.present || data.either || '',
  ...
  // Pull paths
  if (lexicon.paths) {
    ctx.paths = lexicon.paths.map(p => ({ label: p.label }));
  }

  // Pull reframes + analogies from first 2-3 paths (closer needs ammunition)
  if (lexicon.paths) {
    lexicon.paths.slice(0, 3).forEach(p => {
      if (p.reframeQuestions) ctx.reframes.push(...p.reframeQuestions.slice(0, 2));
      if (p.analogies) p.analogies.slice(0, 2).forEach(a => ctx.analogies.push(a.text || a.body || ''));
    });
  }
...
function renderPickerBanner() {
  ...
  if (mode === 'ai') {
    ...
    if (!persona) {
      banner.innerHTML = CardRenderer.renderModeSelectCard('ai') + CardRenderer.renderAIBriefCard(SessionState.getProspectType(), SessionState.getAiPlays());
      return;
    }
...
async function onChipTap(chip) {
  ...
  try {
    ...
    if (mode === 'ai') {
      const aiPlays = SessionState.getAiPlays();
      const persona = SessionState.getPersona();

      if (aiPlays === 'closer') {
        // AI is the COACH, user is the prospect.
        // Show the Layer 1 reference (what excuses YOU'd raise), then kick off AI with an opening question.
        appendCard(CardRenderer.renderLayer1Card(parsed, chip.label));
        // Show a brief "you're playing..." card
        appendCard(`<div class="card card-role-reminder">
          <div class="card-tag tag-purple">🎭 You are: ${escapeHtml(persona.name)} · Type ${persona.type}</div>
          <div class="card-subtitle">The coach is about to open with a question about <strong>${escapeHtml(chip.label)}</strong>. Respond as ${escapeHtml(persona.name)} would — use the Layer 1 phrasings above as inspiration if you need them.</div>
        </div>`);
        // Fire the AI to ask the opening question
        kickOffAICloser(chip, parsed, persona);
      } else {
        // AI is PROSPECT, user is closer (existing behavior)
        appendCard(CardRenderer.renderLayer1Card(parsed, chip.label));
        const helperText = '**Excuse:** ' + chip.label + '\n\nAsk a SCRIPT question (S/C/R/I/P/T) in your own words. The prospect will respond in character.';
        appendCard(CardRenderer.renderCloserInputCard(
          'Ask the prospect about this excuse.',
          helperText
        ));
...
// Kick off the AI closer with an opening question when the chip is tapped
async function kickOffAICloser(chip, parsed, persona) {
  const lexiconContext = buildLexiconContext(parsed, chip.label, persona.type);
  const systemPrompt = AIEngine.buildCloserSystemPrompt(persona, lexiconContext, SessionState.isScoopDeployed());

  // Seed message — instructs AI to open the conversation about this excuse
  const seedMsg = `[The prospect (you are coaching them) just brought up the "${chip.label}" excuse. Open with your first SCRIPT question — pick the letter that best fits where they are. Just your words, no preamble.]`;

  const loadingCard = `<div class="card card-loading" id="card-ai-loading">⏳ Coach is opening...</div>`;
  appendCard(loadingCard);
  scrollToBottom();

  try {
    const result = await AIEngine.generateResponse(systemPrompt, ConversationHistory.getForAPI(), seedMsg);
    // Remove the loading card
    const loader = document.getElementById('card-ai-loading');
    if (loader) loader.remove();

    ConversationHistory.addUserTurn(seedMsg, { hidden: true, kickoff: true });
    ConversationHistory.addAssistantTurn(result.text, { cost: result.cost, aiPlays: 'closer' });
    ...
    appendCard(CardRenderer.renderCloserInputCard(
      'Respond as ' + persona.name + ' (Type ' + persona.type + ').',
      null
    ));
    DrillState.setStage('ai-closer-input');
    triggerAutoSave();
    ...
    const loader = document.getElementById('card-ai-loading');
    if (loader) loader.remove();
    alert('AI failed to open: ' + err.message);
  }
}
...
const DrillFlow = {
  ...
  async sendCloserInput(widgetId) {
    ...
    if (!state || !persona) {
      ...
      if (sendBtn) { sendBtn.disabled = false; sendBtn.textContent = 'Send → see response ▼'; }
      return;
    }

    // Build a rich lexicon context that supports BOTH prompt builders
    const lexiconContext = buildLexiconContext(state.lexicon, state.chip.label, persona.type);

    // Choose which prompt to use based on aiPlays
    const aiPlays = SessionState.getAiPlays();
    const systemPrompt = (aiPlays === 'closer')
      ? AIEngine.buildCloserSystemPrompt(persona, lexiconContext, SessionState.isScoopDeployed())
      : AIEngine.buildSystemPrompt(persona, lexiconContext, SessionState.isScoopDeployed());

    try {
      const result = await AIEngine.generateResponse(
        systemPrompt,
        ConversationHistory.getForAPI().slice(0, -1),
        input
      );
      ConversationHistory.addAssistantTurn(result.text, { cost: result.cost, aiPlays: aiPlays });

      // Render AI message card — use closer voice if AI is the closer, prospect voice if AI is prospect
      appendCard(CardRenderer.renderAIProspectMessageCard(result.text, persona, {
        deliveryMode: aiPlays === 'closer'
          ? 'closer'
          : (SessionState.isScoopDeployed() ? 'fear' : (persona.type === 'A' ? 'typeA' : 'typeB')),
        aiPlays: aiPlays
      }));
      appendCard(CardRenderer.renderCloserInputCard(
        aiPlays === 'closer'
          ? 'Respond as ' + persona.name + ' (your character). Reference the cards if you need help staying in character.'
          : 'Your next move. Reference the framework cards if you need to pick a SCRIPT letter, deploy a reframe, mark concession, or scoop.',
        null
      ));
      DrillState.setStage('ai-closer-input');
      triggerAutoSave();
  ...
  async regenerateLastAIResponse() {
    ...
    try {
      ...
      ConversationHistory.addAssistantTurn(result.text, { cost: result.cost, regenerated: true, aiPlays });
      appendCard(CardRenderer.renderAIProspectMessageCard(result.text, persona, {
        deliveryMode: aiPlays === 'closer'
          ? 'closer'
          : (SessionState.isScoopDeployed() ? 'fear' : (persona.type === 'A' ? 'typeA' : 'typeB')),
  ...
  showScoopReference() {
    ...
    if (!state) return;
    if (!SessionState.isScoopGateUnlocked() && !SessionState.isOverridden()) {
      alert('Scoop gate is locked — clear 2 excuses first, OR mark concession Yes on this one to unlock, OR override from the sweep card.');
    }
    appendCard(CardRenderer.renderScoopDeployCard(state.lexicon, SessionState));
    scrollToBottom();
  },

  // AI mode: mark Yes concession and immediately show sweep card (with Back to picker)
  aiMarkConcessionYes() {
    const state = DrillState.getState();
    if (!state) return;
    DrillState.recordConcession('yes');
    appendToDrillLog(state.chip.id, 'concession', { verdict: 'yes' });
    SessionState.recordExcuseCleared(
      state.chip,
      DrillState.getCurrentPath() || { label: 'AI mode (no explicit path)' },
      state.choices.analogyChosen,
      'yes'
    );
    appendCard(CardRenderer.renderSweepCard(state.lexicon, SessionState));
  ...
  // AI mode: prospect hasn't fully conceded; keep digging in same drill
  aiMarkConcessionPartial() {
    appendCard(CardRenderer.renderResistanceCard('partial'));
    DrillState.setStage('resistance');
    // Also append another closer input card so the conversation can continue
    appendCard(CardRenderer.renderCloserInputCard(
      'Their concession is shaky. Stay in this excuse — try a different SCRIPT letter, a deeper reframe, or a different analogy. Don\'t move on yet.',
      null
    ));
    scrollToBottom();
  }
