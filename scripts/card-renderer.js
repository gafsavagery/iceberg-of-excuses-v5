...
const CardRenderer = (function() {
  ...
  // =================================================================
  function renderAIBriefCard(prospectType) {
    let html = '<div class="card card-ai-brief" id="card-ai-brief">';
    html += '<div class="card-tag tag-purple">🎭 Set up your prospect</div>';
    html += '<div class="card-title">Who are you about to coach?</div>';
    html += '<div class="card-subtitle">Type a brief about a real prospect (for call replay), OR skip to auto-generate a fresh persona. Either way they\'ll be Type ' + prospectType + '.</div>';

    html += '<div class="card-section-label">Optional brief (call replay, specific prospect):</div>';
    html += '<textarea id="ai-brief-input" class="brief-textarea" rows="6" placeholder="Examples:&#10;- \'Marcus, 42, runs a $4M dental practice in Chicago. Married, 2 kids. Said he doesn\'t have time. Sounded controlled but his voice tightened when I asked about his weekends.\'&#10;- \'Botched call from last week: pissed-off ecom owner who burned $50K on a previous coach, kept telling me coaching doesn\'t work.\'&#10;- \'Single mom, 38, fitness brand at $1.5M, sounded overwhelmed, raises money objections constantly.\'"></textarea>';

    html += '<div class="card-action-row">';
    html += '<button class="card-btn primary" onclick="DrillFlow.startAICallWithBrief()">📋 Start call with this brief →</button>';
    html += '<button class="card-btn secondary" onclick="DrillFlow.startAICallAutoPersona()">🎲 Skip — auto-generate persona</button>';
    html += '</div>';
  ...
  function renderCloserInputCard(promptText, helperContext) {
    ...
    // Framework guardrail buttons — let the closer pull up reference cards
    html += '<div class="closer-input-helpers">';
    html += '<button class="helper-btn" onclick="DrillFlow.showSCRIPTReference()">📚 SCRIPT letters</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showPathsReference()">🔵 Path diagnosis</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showAnalogiesReference()">🎯 Reframes & analogies</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showConcessionReference()">✓ Mark concession & sweep</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showScoopReference()">🎯 Deploy scoop</button>';
    html += '</div>';
