...
const CardRenderer = (function() {
  ...
  // =================================================================
  function renderScriptQuestionCard(parsed, letterKey, role) {
    const letterData = parsed.script.letters[letterKey];
    if (!letterData) return '';
    role = role || 'closer';

    const widgetId = 'scriptq-' + letterKey + '-' + Date.now();
    let html = '<div class="card card-script-q" id="card-' + widgetId + '">';
    html += '<div class="card-tag tag-green">🟢 ' + letterKey + ' — ' + esc(letterData.rawHeader.replace(/^[SCRIPT]\s*[—–-]\s*/i, '')) + '</div>';

    if (role === 'prospect') {
      html += '<div class="card-title">The closer is asking you:</div>';
    } else {
    ...
    if (letterData.listenFor && role === 'closer') {
    ...
    // Voice playback widget if voice on (closer's question being spoken to you when prospect-role)
    const shouldPlay = typeof VoiceEngine !== 'undefined' && VoiceEngine.isEnabled() && VoiceEngine.hasApiKey() && role === 'prospect';
    if (shouldPlay) {
      html += VoiceUI.renderPlaybackControls(widgetId);
    }

    html += '<div class="card-action-row">';
    if (role === 'prospect') {
      html += '<button class="card-btn primary" id="advance-' + widgetId + '" onclick="DrillFlow.askedScriptQuestion()">I\'ve heard the question · respond as prospect ▼</button>';
    } else {
    ...
    // Schedule TTS playback in prospect-role
    if (shouldPlay) {
      const speakText = letterData.past || letterData.present || letterData.either || '';
      setTimeout(() => {
        const prefs = VoiceEngine.getVoicePrefs();
        const advanceBtn = document.getElementById('advance-' + widgetId);
        if (prefs.gateAdvance && advanceBtn) advanceBtn.disabled = true;
        VoiceUI.startCardPlayback(widgetId, speakText, {
          voiceId: prefs.voiceForCloser,
          deliveryMode: 'closer',
          onDone: () => { if (advanceBtn) advanceBtn.disabled = false; }
        });
      }, 200);
    }
  ...
  function renderProspectResponseCard(response, prospectType) {
    const widgetId = 'prospect-' + Date.now();
    let html = '<div class="card card-prospect" id="card-' + widgetId + '">';
    ...
    // Voice playback widget (only renders if voice is enabled — handled in start)
    if (typeof VoiceEngine !== 'undefined' && VoiceEngine.isEnabled() && VoiceEngine.hasApiKey()) {
      html += VoiceUI.renderPlaybackControls(widgetId);
    }

    html += '<div class="card-action-row">';
    html += '<button class="card-btn primary" id="advance-' + widgetId + '" onclick="DrillFlow.advanceToPathDiagnosis()">Continue → diagnose path ▼</button>';
    html += '<button class="card-btn secondary" onclick="DrillFlow.tryAnotherScriptLetter()">Try another SCRIPT letter first</button>';
    html += '</div>';
    html += '</div>';

    // Schedule playback to start AFTER card is in DOM
    if (typeof VoiceEngine !== 'undefined' && VoiceEngine.isEnabled() && VoiceEngine.hasApiKey()) {
      setTimeout(() => {
        const prefs = VoiceEngine.getVoicePrefs();
        const voiceId = prospectType === 'A' ? prefs.voiceForA : prefs.voiceForB;
        const advanceBtn = document.getElementById('advance-' + widgetId);
        if (prefs.gateAdvance && advanceBtn) advanceBtn.disabled = true;
        VoiceUI.startCardPlayback(widgetId, response.text, {
          voiceId: voiceId,
          deliveryMode: prospectType === 'A' ? 'typeA' : 'typeB',
          onDone: () => { if (advanceBtn) advanceBtn.disabled = false; }
        });
      }, 200);
    }
  ...
  function renderScoopDeployCard(parsed, sessionState, statedGoal) {
    ...
    const role = sessionState.getRole ? sessionState.getRole() : 'closer';
    ...
    const widgetId = 'scoop-' + Date.now();
    const scoopText = 'So aside from ' + parrotList + ' — what do you feel has actually been in the way of you having your goal?';

    let html = '<div class="card card-scoop-deploy" id="card-' + widgetId + '">';
    html += '<div class="card-tag tag-purple">🎯 SCOOP · the door to the fear</div>';

    if (role === 'prospect') {
      html += '<div class="card-title">The closer is scooping you now:</div>';
    } else {
    ...
    if (parsed.scoop && parsed.scoop.permissionPreamble && role !== 'prospect') {
      html += '<details class="card-details">';
    ...
    // TTS widget if in prospect-role
    const shouldPlay = typeof VoiceEngine !== 'undefined' && VoiceEngine.isEnabled() && VoiceEngine.hasApiKey() && role === 'prospect';
    if (shouldPlay) {
      html += VoiceUI.renderPlaybackControls(widgetId);
    }
    ...
    html += '<div class="card-action-row">';
    if (role === 'prospect') {
      html += '<button class="card-btn primary" id="advance-' + widgetId + '" onclick="DrillFlow.scoopAsked()">Sit in silence · then respond ▼</button>';
    } else {
    ...
    if (shouldPlay) {
      setTimeout(() => {
        ...
        const advanceBtn = document.getElementById('advance-' + widgetId);
        if (prefs.gateAdvance && advanceBtn) advanceBtn.disabled = true;
        VoiceUI.startCardPlayback(widgetId, scoopText, {
          voiceId: prefs.voiceForCloser,
          deliveryMode: 'closer',
          onDone: () => { if (advanceBtn) advanceBtn.disabled = false; }
        });
  ...
  // Start the silence timer (called from app.js AFTER the card is inserted).
  // Returns the interval ID so caller can stop it explicitly.
  function startSilenceTimer() {
    const el = document.getElementById('silence-timer');
    if (!el) return null;
    const start = Date.now();
    el.textContent = '0s';
    el.className = 'silence-timer';
    const interval = setInterval(() => {
      if (!el.isConnected) { clearInterval(interval); return; }
      const seconds = Math.floor((Date.now() - start) / 1000);
      el.textContent = seconds + 's';
      if (seconds >= 10) el.className = 'silence-timer overdue';
      else if (seconds >= 5) el.className = 'silence-timer ready';
    }, 250);
    el.dataset.intervalId = interval;
    return interval;
  }

  // Stop the silence timer and freeze the displayed value.
  ...
  // =================================================================
  function renderNamedFearCard(parsed, sessionState, sampleIndex) {
    ...
    const generic = prospectType === 'A'
      ? ["I think... honestly, I think I'm afraid of finding out I'm not as capable as everyone thinks I am.",
         ...
         "Honestly? I'm scared this is just who I am now and I can't change it.",
         "I'm afraid that if I actually slow down, the whole thing falls apart."]
      : ["I'm scared that if I really try and it doesn't work, I'll have to accept I can't do this.",
         ...
         "I think I'm afraid that I'm just... not the person I thought I was.",
         "I'm scared I've been lying to myself for years and I can't keep doing it."];

    const usingLexicon = fears.length > 0;
    const sourceArr = usingLexicon ? fears : generic;

    // Pick fear at sampleIndex or random
    let fearText, fearIndex;
    if (sampleIndex !== undefined && sampleIndex >= 0 && sampleIndex < sourceArr.length) {
      fearIndex = sampleIndex;
    } else {
      fearIndex = Math.floor(Math.random() * sourceArr.length);
    }
    fearText = sourceArr[fearIndex];
    sessionState.recordNamedFear(fearText, fearIndex, prospectType);

    const widgetId = 'namedfear-' + Date.now();
    let html = '<div class="card card-named-fear" id="card-' + widgetId + '">';
    ...
    html += 'Fear ' + (fearIndex + 1) + ' of ' + sourceArr.length;
    html += usingLexicon
      ? ' · drawn from <strong>this excuse\'s lexicon</strong> (Type ' + prospectType + ' samples)'
      : ' · drawn from <strong>generic fallback</strong> (this lexicon has no scoop section yet)';
    html += '</div>';
    ...
    // PICK ANOTHER button
    html += '<div class="card-action-row" style="margin-top: 8px;">';
    html += '<button class="card-btn secondary" onclick="DrillFlow.pickAnotherFear()">🔄 Pick another fear from this lexicon</button>';
    html += '</div>';

    // === FULL FEAR LEXICON VISIBLE ===
    // Show both Type A and Type B fears from this excuse for reference,
    // plus the foundational core-fears reminder.
    html += '<details class="card-details" style="margin-top: 12px;" ' + (sourceArr.length <= 3 ? 'open' : '') + '>';
    html += '<summary>📚 All Type ' + prospectType + ' fears in this lexicon (' + sourceArr.length + ')</summary>';
    html += '<div class="card-details-body">';
    html += '<ul class="fear-menu-list">';
    sourceArr.forEach((f, i) => {
      const isPicked = i === fearIndex;
      html += '<li class="fear-menu-item' + (isPicked ? ' picked' : '') + '">';
      if (isPicked) html += '<span class="fear-menu-marker">▸ shown above</span> ';
      html += esc(f);
      html += '</li>';
    });
    html += '</ul>';
    ...
    // Show the OPPOSITE type for reference too, if both exist in lexicon
    const otherType = prospectType === 'A' ? 'B' : 'A';
    const otherFears = otherType === 'A' ? (parsed.scoop?.typeAFears || []) : (parsed.scoop?.typeBFears || []);
    if (otherFears.length > 0 && usingLexicon) {
      html += '<details class="card-details">';
      html += '<summary>👁 Reference: Type ' + otherType + ' fears for the same excuse (' + otherFears.length + ')</summary>';
      html += '<div class="card-details-body">';
      html += '<ul class="fear-menu-list">';
      otherFears.forEach(f => html += '<li class="fear-menu-item">' + esc(f) + '</li>');
      html += '</ul>';
      html += '</div>';
      html += '</details>';
    }

    // Core fears reference (the 8 universal fear families)
    html += '<details class="card-details">';
    html += '<summary>🔍 Core fears reference — the universal menu</summary>';
    html += '<div class="card-details-body fear-reference">';
    html += '<p><em>Every named fear traces to one of these. Use this to triangulate what they actually said.</em></p>';
    html += '<ul class="fear-menu-list">';
    [
      ['Fraud exposure', '"I got lucky. I\'m not actually what I appear to be."'],
      ['Self-trust collapse', '"If I get screwed over again, I\'ll never trust my judgment."'],
      ['Identity dissolution', '"Who am I without this business / this hard-ass operating style?"'],
      ['Public humiliation', '"I\'ll look stupid. My peers will see I couldn\'t pull it off."'],
      ['Financial / stability', '"Lose money, can\'t recover, team suffers, family suffers."'],
      ['Rejection', '"If I ask and they say no, what does that mean about me?"'],
      ['Wasted time', '"If I try now and fail, I\'ve wasted everything."'],
      ['Loss of meaning', '"If I\'m not building / striving / sacrificing — what\'s the point?"']
    ].forEach(([name, q]) => {
      html += '<li class="fear-menu-item"><strong>' + name + '</strong> — ' + esc(q) + '</li>';
    });
    html += '</ul>';
    html += '</div>';
    html += '</details>';
    ...
    html += '<div class="card-action-row">';
    html += '<button class="card-btn primary" id="advance-' + widgetId + '" onclick="DrillFlow.advanceToAhaCheck()">Continue → check aha type ▼</button>';
    html += '</div>';
    html += '</div>';

    // Schedule playback after card is in DOM
    if (typeof VoiceEngine !== 'undefined' && VoiceEngine.isEnabled() && VoiceEngine.hasApiKey()) {
      setTimeout(() => {
        const prefs = VoiceEngine.getVoicePrefs();
        const voiceId = prospectType === 'A' ? prefs.voiceForA : prefs.voiceForB;
        const advanceBtn = document.getElementById('advance-' + widgetId);
        if (prefs.gateAdvance && advanceBtn) advanceBtn.disabled = true;
        VoiceUI.startCardPlayback(widgetId, fearText, {
          voiceId: voiceId,
          deliveryMode: 'fear',
          onDone: () => { if (advanceBtn) advanceBtn.disabled = false; }
        });
  ...
  // =================================================================
  // CALL START — Type A/B selector + opener + role selector
  // =================================================================
  function renderCallStartCard(prospectType, role) {
    role = role || 'closer';
    ...
    // Role selector
    html += '<div class="card-section-label">Your role this call:</div>';
    html += '<div class="role-selector">';
    html += '<button class="role-btn ' + (role === 'closer' ? 'active' : '') + '" onclick="toggleRole(\'closer\')">';
    html += '<div class="role-name">👤 Play the closer</div>';
    html += '<div class="role-desc">You speak SCRIPT questions, reframes, analogies. Sim plays the prospect.</div>';
    html += '</button>';
    html += '<button class="role-btn ' + (role === 'prospect' ? 'active' : '') + '" onclick="toggleRole(\'prospect\')">';
    html += '<div class="role-name">🎭 Play the prospect</div>';
    html += '<div class="role-desc">Sim speaks closer\'s questions to you. You respond as a prospect would.</div>';
    html += '</button>';
    html += '</div>';
  ...
  // =================================================================
  // AI MODE — MODE SELECT CARD (replaces basic call start in AI mode)
  // =================================================================
  function renderModeSelectCard(currentMode) {
    let html = '<div class="card card-mode-select" id="card-mode-select">';
    html += '<div class="card-tag tag-purple">⚙ Pre-call setup</div>';
    html += '<div class="card-title">Pick your practice mode</div>';
    html += '<div class="card-subtitle">Both modes use the same cards & framework. The difference is what the prospect says.</div>';

    html += '<div class="mode-options">';
    html += '<button class="mode-option ' + (currentMode === 'lexicon' ? 'active' : '') + '" onclick="DrillFlow.setMode(\'lexicon\')">';
    html += '<div class="mode-name">📜 Lexicon mode</div>';
    html += '<div class="mode-desc">Sim plays the prospect using pre-written samples from the lexicon. Fast, free, deterministic. Good for drill reps.</div>';
    html += '</button>';

    html += '<button class="mode-option ' + (currentMode === 'ai' ? 'active' : '') + '" onclick="DrillFlow.setMode(\'ai\')">';
    html += '<div class="mode-name">🤖 AI prospect mode</div>';
    html += '<div class="mode-desc">Claude plays a live prospect. Real conversation. You speak or type, they respond in character. Costs Anthropic + ElevenLabs credits per session.</div>';
    html += '</button>';
    html += '</div>';
  ...
  // =================================================================
  // AI BRIEF CARD — paste/type the prospect persona before call starts
  // =================================================================
  function renderAIBriefCard(prospectType, aiPlays) {
    aiPlays = aiPlays || 'prospect';
    let html = '<div class="card card-ai-brief" id="card-ai-brief">';
    html += '<div class="card-tag tag-purple">🎭 Set up the call</div>';
    html += '<div class="card-title">Who you\'re practicing as</div>';

    html += '<div class="role-selector" style="margin-bottom: 14px;">';
    html += '<button class="role-btn ' + (aiPlays === 'prospect' ? 'active' : '') + '" onclick="DrillFlow.setAiPlays(\'prospect\')">';
    html += '<div class="role-name">👤 I\'m the closer (default)</div>';
    html += '<div class="role-desc">AI plays a Type ' + prospectType + ' prospect using the lexicon. You ask SCRIPT questions, run reframes, sweep, scoop.</div>';
    html += '</button>';
    html += '<button class="role-btn ' + (aiPlays === 'closer' ? 'active' : '') + '" onclick="DrillFlow.setAiPlays(\'closer\')">';
    html += '<div class="role-name">🎭 I\'m the prospect</div>';
    html += '<div class="role-desc">AI plays the coach. They run the framework on you. You respond as the persona in your brief.</div>';
    html += '</button>';
    html += '</div>';

    html += '<div class="card-section-label">Brief (optional, for call replay or specific prospects):</div>';
    html += '<textarea id="ai-brief-input" class="brief-textarea" rows="6" placeholder="Examples:&#10;- \'Marcus, 42, runs a $4M dental practice. Married, 2 kids. Sounded controlled but tightened up about weekends.\'&#10;- \'Single mom, 38, fitness brand at $1.5M, raises money objections constantly.\'&#10;- (Leave blank to auto-generate a random Type ' + prospectType + ' persona)"></textarea>';

    html += '<div class="card-action-row">';
    html += '<button class="card-btn primary" onclick="DrillFlow.startAICallWithBrief()">📋 Start with this brief →</button>';
    html += '<button class="card-btn secondary" onclick="DrillFlow.startAICallAutoPersona()">🎲 Skip — auto-generate</button>';
    html += '</div>';
  ...
  // =================================================================
  // PERSONA CARD — shows the closer who they're talking to
  // =================================================================
  function renderPersonaCard(persona) {
    if (!persona) return '';
    let html = '<div class="card card-persona" id="card-persona">';
    html += '<div class="card-tag tag-purple">📞 On the line: Type ' + persona.type + '</div>';
    html += '<div class="card-title">' + esc(persona.name) + ' · ' + esc(String(persona.age)) + '</div>';
    html += '<div class="persona-business">' + esc(persona.business) + '</div>';

    if (persona.source === 'brief' && persona.raw) {
      html += '<details class="card-details" open>';
      html += '<summary>Brief you provided</summary>';
      html += '<div class="card-details-body persona-brief">' + esc(persona.raw) + '</div>';
      html += '</details>';
    } else {
      html += '<details class="card-details">';
      html += '<summary>Backstory (click to expand)</summary>';
      html += '<div class="card-details-body">';
      html += '<p>' + esc(persona.background) + '</p>';
      html += '<p><em>Real driver:</em> ' + esc(persona.realMotivation) + '</p>';
      html += '</details>';
    }

    html += '<div class="card-action-row">';
    html += '<button class="card-btn primary" onclick="DrillFlow.aiCallReady()">Ready · go to picker ▼</button>';
    html += '<button class="card-btn secondary" onclick="DrillFlow.regeneratePersona()">🔄 Different persona</button>';
    html += '</div>';
  ...
  // =================================================================
  // AI PROSPECT MESSAGE CARD — what the AI just said
  // =================================================================
  function renderAIProspectMessageCard(messageText, persona, options) {
    options = options || {};
    const widgetId = 'ai-msg-' + Date.now();
    let html = '<div class="card card-ai-message" id="card-' + widgetId + '">';
    html += '<div class="card-tag tag-prospect">👤 ' + esc(persona.name) + ' (Type ' + persona.type + ')</div>';
    html += '<blockquote class="card-ai-quote">' + esc(messageText) + '</blockquote>';
    ...
    // History toggle
    if (typeof ConversationHistory !== 'undefined' && ConversationHistory.getLength() > 2) {
      html += '<details class="card-details history-toggle">';
      html += '<summary>📜 Earlier in this call (' + ConversationHistory.getLength() + ' turns)</summary>';
      html += '<div class="card-details-body history-body">';
      ConversationHistory.getAll().slice(0, -1).forEach(turn => {
        const speaker = turn.role === 'user' ? 'You' : persona.name;
        const cls = turn.role === 'user' ? 'history-turn-user' : 'history-turn-ai';
        html += '<div class="history-turn ' + cls + '"><strong>' + esc(speaker) + ':</strong> ' + esc(turn.content) + '</div>';
      });
      html += '</div>';
      html += '</details>';
    }
    ...
    html += '<div class="card-action-row">';
    html += '<button class="card-btn secondary" onclick="DrillFlow.regenerateLastAIResponse()">🔄 Regenerate response</button>';
    html += '</div>';
    ...
    if (typeof VoiceEngine !== 'undefined' && VoiceEngine.isEnabled() && VoiceEngine.hasApiKey()) {
      setTimeout(() => {
        const prefs = VoiceEngine.getVoicePrefs();
        const voiceId = persona.type === 'A' ? prefs.voiceForA : prefs.voiceForB;
        const deliveryMode = options.deliveryMode || (persona.type === 'A' ? 'typeA' : 'typeB');
        VoiceUI.startCardPlayback(widgetId, messageText, {
          voiceId: voiceId,
  ...
  // =================================================================
  // CLOSER INPUT CARD — mic + textarea for the closer to respond
  // =================================================================
  function renderCloserInputCard(promptText, helperContext) {
    const widgetId = 'closer-in-' + Date.now();
    let html = '<div class="card card-closer-input" id="card-' + widgetId + '">';
    html += '<div class="card-tag tag-blue">🎤 Your turn — speak or type</div>';
    if (promptText) {
      html += '<div class="card-subtitle">' + esc(promptText) + '</div>';
    }

    if (helperContext) {
      html += '<details class="card-details">';
      html += '<summary>📋 Card reference (what the framework suggests)</summary>';
      html += '<div class="card-details-body">' + mdLite(helperContext) + '</div>';
      html += '</details>';
    }

    // Framework guardrail buttons
    html += '<div class="closer-input-helpers">';
    html += '<button class="helper-btn" onclick="DrillFlow.showSCRIPTReference()">📚 SCRIPT</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showPathsReference()">🔵 Paths</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showAnalogiesReference()">🎯 Reframes</button>';
    html += '<button class="helper-btn" onclick="DrillFlow.showScoopReference()">🎯 Scoop</button>';
    html += '</div>';

    // Mic + textarea
    if (typeof VoiceEngine !== 'undefined' && VoiceEngine.micSupported()) {
      html += VoiceUI.renderMicWidget(widgetId);
      html += '<div class="closer-input-or">or type:</div>';
    }
    html += '<textarea id="closer-text-' + widgetId + '" class="closer-textarea" rows="3" placeholder="Type your response here..."></textarea>';

    html += '<div class="card-action-row">';
    html += '<button class="card-btn primary" onclick="DrillFlow.sendCloserInput(\'' + widgetId + '\')">Send → see prospect response ▼</button>';
    html += '</div>';

    // Prominent "I cleared this excuse" button — drives the sweep flow
    html += '<div class="closer-finish-row">';
    html += '<div class="closer-finish-label">When you feel they\'ve conceded this excuse:</div>';
    html += '<button class="card-btn secondary closer-finish-btn" onclick="DrillFlow.aiMarkConcessionYes()">✓ Concession Yes — sweep & continue ▶</button>';
    html += '<button class="card-btn secondary closer-finish-btn-resist" onclick="DrillFlow.aiMarkConcessionPartial()">⚠ Partial concession — keep digging</button>';
    html += '</div>';
  ...
  return {
    ...
    renderAIBriefCard,
    renderPersonaCard,
    renderAIProspectMessageCard,
    renderCloserInputCard,
    renderModeSelectCard
  };
})();
