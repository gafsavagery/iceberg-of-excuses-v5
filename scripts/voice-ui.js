...
const VoiceUI = (function() {
  ...
  function renderSettingsPanel() {
    ...
    const hasAIKey = typeof AIEngine !== 'undefined' && AIEngine.hasApiKey();
    const aiModel = typeof AIEngine !== 'undefined' ? AIEngine.getModel() : 'sonnet';
    const aiModels = typeof AIEngine !== 'undefined' ? AIEngine.getModels() : {};
    const aiCost = typeof AIEngine !== 'undefined' ? AIEngine.getSessionCost() : 0;
    ...
    // === ANTHROPIC AI SECTION (new) ===
    html += '<div class="panel-section">';
    html += '<div class="panel-label">🤖 Anthropic API (for AI prospect mode)</div>';
    html += '<div class="panel-help">Required for AI prospect mode. Get one at console.anthropic.com. Stored in browser only.</div>';
    html += '<div class="key-row">';
    html += '<input type="password" id="ai-key-input" class="panel-input" placeholder="' + (hasAIKey ? '••••••••• (saved)' : 'sk-ant-...') + '" autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false">';
    html += '<button class="panel-btn" onclick="VoiceUI.saveAIKey()">Save</button>';
    html += '</div>';
    html += '<div class="key-actions">';
    if (hasAIKey) {
      html += '<button class="panel-btn small" onclick="VoiceUI.testAIKey()">🧪 Test key</button>';
      html += '<button class="panel-btn small danger" onclick="VoiceUI.clearAIKey()">🗑 Clear key</button>';
    }
    html += '</div>';
    html += '<div class="key-status" id="ai-key-status"></div>';
    // Model picker
    html += '<div class="voice-pick-row" style="margin-top: 12px;">';
    html += '<label>Model</label>';
    html += '<select class="panel-select" id="ai-model-pick">';
    Object.entries(aiModels).forEach(([key, mod]) => {
      const sel = key === aiModel ? ' selected' : '';
      html += '<option value="' + key + '"' + sel + '>' + esc(mod.label) + '</option>';
    });
    html += '</select>';
    html += '</div>';
    // Cost display
    html += '<div class="cost-display">Session cost so far: <strong>$' + aiCost.toFixed(4) + '</strong> <button class="panel-btn small" onclick="VoiceUI.resetAICost()">Reset</button></div>';
    html += '</div>';

    // === ELEVENLABS SECTION ===
    html += '<div class="panel-section">';
    html += '<div class="panel-label">🔊 ElevenLabs API (for voice playback)</div>';
    ...
    html += '</select>';
    html += '<button class="panel-btn small" onclick="VoiceUI.previewVoice(\'a\')">▶</button>';
    html += '</div>';
    ...
    html += '</select>';
    html += '<button class="panel-btn small" onclick="VoiceUI.previewVoice(\'b\')">▶</button>';
    html += '</div>';
    ...
    html += '</select>';
    html += '<button class="panel-btn small" onclick="VoiceUI.previewVoice(\'closer\')">▶</button>';
    html += '</div>';
    html += '</div>';
  ...
  async function testKey() {
    ...
    try {
      ...
      let msg = '✓ Key works';
      if (data?.voices_available) msg += ' · ' + data.voices_available + ' voices accessible';
  ...
  function savePrefsAndClose() {
    ...
    const aiModelSel = document.getElementById('ai-model-pick');
    ...
    if (aiModelSel && typeof AIEngine !== 'undefined') AIEngine.setModel(aiModelSel.value);
    closeSettings();
  }
  ...
  function saveAIKey() {
    if (typeof AIEngine === 'undefined') { showAIKeyStatus('AI engine not loaded', 'error'); return; }
    const input = document.getElementById('ai-key-input');
    if (!input) return;
    const val = (input.value || '').trim();
    if (!val) { showAIKeyStatus('Empty — paste your key first', 'warn'); return; }
    AIEngine.setApiKey(val);
    showAIKeyStatus('✓ Key saved to this browser', 'ok');
    input.value = '';
    setTimeout(() => {
      const panel = document.getElementById('voice-settings-panel');
      if (panel) panel.outerHTML = renderSettingsPanel();
  ...
  async function testAIKey() {
    if (typeof AIEngine === 'undefined') { showAIKeyStatus('AI engine not loaded', 'error'); return; }
    showAIKeyStatus('Testing key...', 'pending');
    try {
      await AIEngine.testKey();
      showAIKeyStatus('✓ Anthropic key works', 'ok');
    } catch (err) {
      showAIKeyStatus('✗ ' + err.message, 'error');
    }
  }

  function clearAIKey() {
    if (typeof AIEngine === 'undefined') return;
    if (!confirm('Clear the saved Anthropic API key? You\'ll need to paste it again to use AI mode.')) return;
  ...
  function resetAICost() {
    if (typeof AIEngine === 'undefined') return;
    AIEngine.resetSessionCost();
    const panel = document.getElementById('voice-settings-panel');
    if (panel) panel.outerHTML = renderSettingsPanel();
  }
  ...
  return {
    ...
    saveAIKey, testAIKey, clearAIKey, resetAICost,
