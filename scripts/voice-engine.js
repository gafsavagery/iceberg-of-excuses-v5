...
const VoiceEngine = (function() {
  ...
  async function testKey() {
    ...
    // Test using /v1/voices endpoint — works with any key that has TTS permission
    // (the /v1/user endpoint requires "User Info" permission which not all keys have)
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) throw new Error('Invalid API key' + detail);
      if (response.status === 403) throw new Error('Key missing required permissions' + detail);
      throw new Error('HTTP ' + response.status + detail);
    }
    const data = await response.json();
    // Return a minimal "ok" — voices list doesn't have subscription info
    return { voices_available: (data.voices || []).length };
  }
